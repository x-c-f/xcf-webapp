...
<div class="row">
    ...
    <transition name="push">
      ...
      <template v-for="TagGroup in tags.Tags">
            ...
            <template v-for="filter in TagGroup.TagGroup.TagList">
             <TagLabel :filter="filter" :FILTERS="FILTERS" :TagGroup="TagGroup" :key="filter.Long+':'+ TagGroup.TagGroup.type"/>
          </template>
      </template>
    </div>
  </transition>
  ...
  <transition name="push">
    <div class="TagsFilter row" v-if="show">
      <div class="col-xs-3" v-for="TagGroup in tags.Tags">
        <h5 class="text-left">{{ TagGroup.TagGroup.type }}</h5>
        <div class="checkbox" v-for="filter in TagGroup.TagGroup.TagList">
          <label :class="{disabled: filter.disabled, bolder: !filter.disabled}">
             <input type="checkbox"
              :value="filter.Long"
