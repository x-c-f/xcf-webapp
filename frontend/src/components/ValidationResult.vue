<template>
  <div class="panel panel-default" :class="{'last': isLast}">
    <div class="panel-heading">
      <h5 class="panel-title" v-on:click="toggle()" :title="errorCategory.constraintID">
        <span v-for="(tag, index) in tagslist">
          <span v-show="tag === 'ERROR'" class="mr15 label label-danger"> ERROR </span>
          <span v-show="tag === 'WARNING'" class="mr15 label label-warning"> WARNING </span>
          <span v-show="tag === 'INFO'" class="mr15 label label-info"> INFO </span>
        </span>
        ...
        <div class="row">
          <div class="col-xs-3">
            <ul class="nav nav-pills nav-stacked text-left">
                ...
                <li v-if="errorCategory.longUserDesc || (errorCategory.tags && errorCategory.tags.length > 0)" :class="{ active: activeIndex === 1 }">
                ...
                <li :class="{ active: activeIndex === 2 }">
                  <a  v-on:click="setActive(2)">
                    Specifications
                    <span class="badge badge-default">{{ errorCategory.specs.length }}</span>
                  </a>
                </li>
                ...
                <li :class="{ active: activeIndex === 3 }">
                  <a  v-on:click="setActive(3)">
                    Error Details
                    <span class="badge badge-default">{{ errorCategory.errors.length }}</span>
          ...
          <div class="col-xs-9" v-if="activeIndex === 0">
            <div v-if="errorCategory.shortUserDesc">
              ...
              <button class="btn btn-xs btn-outline"
                ...
                :title="'Violates the ' + spec.name + ' with ' + spec.errorLevel"
                :class="{
                  'filtered': FILTERS['Specifications'].length > 0 && FILTERS['Specifications'].indexOf(spec.nameAcronym) === -1,
          ...
          <div class="col-xs-9" v-if="activeIndex === 2">
            <Specifications :FILTERS="FILTERS" :specifications="errorCategory.specs"/>
          </div>
